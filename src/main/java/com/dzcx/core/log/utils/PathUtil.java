...
import com.dzcx.core.log.PluginAgent;
...
/**
 * user：yeszhjian on 2019/1/4 11:17
...
public class PathUtil {
    ...
    public static Object getDataObj(Object obj, String dataPath) {
        ...
        for (int j = 0; j < paths.length; j++) {
            String path = paths[j];
            Log.d("LazierTracker", "path = " + path);
            switch (path) {
                case Constants.START_THIS:  //this
                    refer = obj;
                    break;
                case Constants.START_ITEM:  //item
                    ...
                    break;
                case Constants.KEY_CONTEXT: //context
                    refer = ((View) refer).getContext();
                    break;
                case Constants.KEY_PARENT:  //parent
        ...
        if (aliveFragments != null) {
            ...
            while (iterator.hasNext()) {
                ...
                if (viewCode == child.hashCode()) {
                    element.append("/").append(fragName).append(validIndexSegment);
